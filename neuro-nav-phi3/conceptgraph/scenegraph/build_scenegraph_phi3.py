"""
Build a scene graph using Vision-Language Models (Phi-3-Vision)
This replaces the YOLO+CLIP+LLaVA+GPT-4 pipeline with modern VLMs.

Key changes from build_scenegraph_cfslam.py:
- Phi-3-Vision for captioning (replaces LLaVA)
- Phi-3-Vision for caption refinement (replaces GPT-4)
- Phi-3-Vision for relationship extraction (replaces GPT-4)
"""
...
class ProgramArgs:
    ...
    # VLM model names
    phi3_model: str = "microsoft/Phi-3-vision-128k-instruct"
...
def extract_node_captions(args):
    """
    Extract captions using Phi-3-Vision (replaces LLaVA).
    """
    from conceptgraph.vlm.phi3_model import Phi3VisionModel
    ...
    # Initialize Phi-3-Vision
    console = rich.console.Console()
    phi3 = Phi3VisionModel(
        model_name=args.phi3_model,
        device=args.device,
        load_in_8bit=True,  # Enable 8-bit quantization to fit in 7.62GB GPU
    )
    print("Phi-3-Vision initialized...")

    # Directories to save features and captions
    savedir_feat = Path(args.cachedir) / "cfslam_feat_phi3"
    savedir_feat.mkdir(exist_ok=True, parents=True)
    savedir_captions = Path(args.cachedir) / "cfslam_captions_phi3"
    savedir_captions.mkdir(exist_ok=True, parents=True)
    savedir_debug = Path(args.cachedir) / "cfslam_captions_phi3_debug"
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf, leave=False):
            ...
            try:
                caption = phi3.caption_image(image_crop_modified)
                console.print(f"[bold green]Phi-3-Vision:[/bold green] {caption}")
                captions.append(caption)
                
                # Note: Phi-3-Vision handles captioning internally
        ...
        # Features not needed with Phi-3-Vision (it handles everything internally)
    ...
    # Save captions
    with open(Path(args.cachedir) / "cfslam_phi3_captions.json", "w", encoding="utf-8") as f:
...
def refine_node_captions(args):
    """
    Refine captions using Phi-3-Vision (replaces GPT-4).
    """
    from conceptgraph.slam.slam_classes import MapObjectList
    from conceptgraph.vlm.phi3_model import Phi3VisionModel

    # Load the captions
    caption_file = Path(args.cachedir) / "cfslam_phi3_captions.json"
    ...
    responses_savedir = Path(args.cachedir) / "cfslam_phi3_responses"
    ...
    for _i in trange(len(captions)):
        ...
        # Refine captions using Phi-3-Vision
        try:
            result = phi3.refine_caption(_caption["captions"])
    ...
    # Save all responses
    with open(Path(args.cachedir) / "cfslam_phi3_responses.pkl", "wb") as f:
...
def build_scenegraph(args):
    ...
    response_dir = Path(args.cachedir) / "cfslam_phi3_responses"
    responses = []
    object_tags = []
    ...
    for idx in range(len(scene_map)):
        ...
        with open(response_dir / f"{idx}.json", "r") as f:
            ...
            try:
                ...
                _d["response"] = {
                    'summary': f'Phi-3-Vision response failed',
                    'possible_tags': ['invalid'],
                    'object_tag': 'invalid'
                }
            responses.append(_d)
    ...
    # Extract relationships using Phi-3-Vision
    minimum_spanning_trees = []
    relations = []
    
    if len(labels) != 0:
        from conceptgraph.vlm.phi3_model import Phi3VisionModel
        phi3 = Phi3VisionModel(model_name=args.phi3_model, device=args.device, load_in_8bit=True)
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    try:
                        result = phi3.extract_object_relationships(
                            images=[],  # Could add images here
                            object1_info=object1_info,
                            object2_info=object2_info,
                        )
...
def main():
    ...
    print(f"Phi-3-Vision model: {args.phi3_model}")
