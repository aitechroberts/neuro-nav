...
from transformers import AutoProcessor, AutoModelForVision2Seq  # or AutoModelForCausalLM depending on HF docs

class SmolVLMClient(PaliGemmaClient):
    def __init__(
        self,
        ...
        # Reuse the prompt handling from PaliGemmaClient
        ...
        # Prompts from Hydra config
        self.prompts = OmegaConf.to_container(prompts) if prompts is not None else {}

        # SmolVLM processor + model (exact class from HF docs)
        self.processor = AutoProcessor.from_pretrained(model_name)
        self.model = AutoModelForVision2Seq.from_pretrained(
            model_name,
            torch_dtype=torch.bfloat16,  # or fp16 if recommended
        ).to(self.device)
        self.model.eval()

        print(f"[SmolVLM] Loaded {model_name} on {self.device}")


_smolvlm_client: Optional[SmolVLMClient] = None
...
def get_qwen3vl_client(
    ...
    global _smolvlm_client
    if _smolvlm_client is None or force_new:
        _smolvlm_client = SmolVLMClient(
            model_name=model_name,
            device=device,
            prompts=prompts,
        )
    return_qwen3vl_client
