...
# from conceptgraph.utils.vis import (  # REMOVED for headless batch
...
@hydra.main(version_base=None, config_path="../../hydra_configs", config_name="batch/batch_test_local")
def main(cfg: DictConfig):
    ...
    # Enhanced WandB initialization for batch jobs
    wandb_config = cfg_to_dict(cfg)
    run_name = f"{cfg.scene_id}_{cfg.exp_suffix}"
    
    owandb.init(
        project="concept-graphs",
        name=run_name,
        group=cfg.exp_suffix,
        tags=[cfg.scene_id, "batch-vlm"],
        config=wandb_config,
    )
    cfg = process_cfg(cfg)
    ...
    for frame_idx in trange(len(dataset)):
        ...
        # REMOVED: Optional render (usually off in batch)
        # if cfg.vis_render: ...
    ...
    # REMOVED: if run_detections and cfg.save_video:
