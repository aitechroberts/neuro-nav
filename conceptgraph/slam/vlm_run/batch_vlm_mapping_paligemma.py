'''
Batch VLM mapping script using PaliGemma for fully local inference.
Replaces OpenAI GPT-4 calls with PaliGemma-3B for:
- Object captioning
...
# =============================================================================
# PaliGemma imports (replaces OpenAI)
# =============================================================================
from conceptgraph.utils.vlms.vlm_paligemma import (
    PaliGemmaClient,
    get_paligemma_client,
...
# =============================================================================
# VLM Edge/Caption Generation using PaliGemma
# =============================================================================
...
@hydra.main(version_base=None, config_path="../../hydra_configs", config_name="batch/batch_vlm_mapping_paligemma")
def main(cfg: DictConfig):
    ...
    if run_detections:
        print("\n".join(["Running detections with PaliGemma..."] * 3))
    ...
    # For hf-hub: timm/PE-Core-T-16-384 you do NOT pass a `pretrained=` tag;
    # the model_name + cache_dir are enough.
    ...
    # Tokenizer must use the same hf-hub model id
    clip_tokenizer = open_clip.get_tokenizer(model_name)

    detection_model.set_classes(obj_classes.get_classes_arr())
    # =========================================================================
    # PALIGEMMA INITIALIZATION - Pass prompts from Hydra config
    # =========================================================================
    paligemma = None
    if cfg.make_edges:
        ...
        paligemma = get_paligemma_client(
    ...
    for frame_idx in trange(len(dataset)):
        ...
        if run_detections:
            ...
            # =========================================================================
            # PALIGEMMA VLM CALLS (uses prompts from config)
            # =========================================================================
            if paligemma is not None:
                labels, edges, edge_image, captions = make_vlm_edges_and_captions_paligemma(
                    image,
                    curr_det,
                    obj_classes,
                    detection_class_labels,
    ...
    # =========================================================================
    # PALIGEMMA CAPTION CONSOLIDATION (uses prompts from config)
    # =========================================================================
    if cfg.make_edges and paligemma is not None:
        print("[PaliGemma] Consolidating captions...")
        for obj in objects:
            obj_captions = obj.get("captions", [])[:20]
            if obj_captions:
                # consolidate_captions uses paligemma.prompts internally
                consolidated_caption = consolidate_captions(paligemma, obj_captions)
