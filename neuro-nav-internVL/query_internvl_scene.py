#!/usr/bin/env python3
"""
Query InternVL Scene - Example script for querying the scene graph using InternVL2-2B

This demonstrates how to use the InternVL2-based scene graph for robot navigation tasks.
"""
...
def query_scene_interactive(internvl_model, scene_graph, image_path=None):
    """Interactive query session"""
    print("\n" + "="*70)
    print("InternVL2 Scene Query Interface")
    print("="*70)
    ...
    while True:
        try:
            ...
            print("\nThinking...")
            answer = internvl_model.query_scene(query, context, image)
            print(f"\n{answer}\n")
...
def query_scene_single(internvl_model, scene_graph, query, image_path=None):
    """Single query mode"""
    context = build_scene_context(scene_graph)
...
def main():
    parser = argparse.ArgumentParser(description="Query InternVL2-based scene graph")
    parser.add_argument(
        '--scene-graph',
        type=str,
    ...
    if not args.scene_graph:
        ...
        if possible_paths:
            ...
            print("\nRun the InternVL pipeline first:")
            print("  ./run_internvl_pipeline.sh")
            return 1
    ...
    print(f"\nInitializing InternVL2 model: {args.model}")
    internvl = InternVL2Model(model_name=args.model, device=args.device)
    ...
    if args.query:
        # Single query mode
        print(f"\nQuery: {args.query}")
        answer = query_scene_single(internvl, scene_graph, args.query, args.image)
        print(f"\nAnswer:\n{answer}\n")
    else:
        # Interactive mode
        query_scene_interactive(internvl, scene_graph, args.image)
