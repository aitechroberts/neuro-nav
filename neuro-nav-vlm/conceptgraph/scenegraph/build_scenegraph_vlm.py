"""
Build a scene graph using Vision-Language Models (Florence-2 + Qwen2-VL)
...
- Qwen2-VL for caption refinement (replaces GPT-4)
- Qwen2-VL for relationship extraction (replaces GPT-4)
...
def extract_node_captions(args):
    """
    Extract captions using Qwen2-VL (replaces LLaVA and Florence-2).
    """
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf, leave=False):
            ...
            try:
                caption = qwen.caption_image(image_crop_modified, detail_level='detailed')
                console.print(f"[bold green]Qwen2-VL:[/bold green] {caption}")
                captions.append(caption)
                
                # Note: Qwen2-VL embeddings can be added here if needed
        ...
        # Features not needed with Qwen2-VL (it handles everything internally)
    ...
    # Save captions
    with open(Path(args.cachedir) / "cfslam_qwen_captions.json", "w", encoding="utf-8") as f:
...
def refine_node_captions(args):
    """
    Refine captions using Qwen2-VL (replaces GPT-4).
    """
    from conceptgraph.slam.slam_classes import MapObjectList
    from conceptgraph.vlm.qwen2vl_model import Qwen2VLModel

    # Load the captions
    caption_file = Path(args.cachedir) / "cfslam_qwen_captions.json"
    ...
    responses_savedir = Path(args.cachedir) / "cfslam_qwen_responses"
    ...
    for _i in trange(len(captions)):
        ...
        # Refine captions using Qwen2-VL
        try:
            result = qwen.refine_caption(image_crop, _caption["captions"])
    ...
    # Save all responses
    with open(Path(args.cachedir) / "cfslam_qwen_responses.pkl", "wb") as f:
...
def build_scenegraph(args):
    ...
    response_dir = Path(args.cachedir) / "cfslam_qwen_responses"
    responses = []
    object_tags = []
    ...
    for idx in range(len(scene_map)):
        ...
        with open(response_dir / f"{idx}.json", "r") as f:
            ...
            try:
                ...
                _d["response"] = {
                    'summary': f'Qwen2-VL response failed',
                    'possible_tags': ['invalid'],
                    'object_tag': 'invalid'
                }
            responses.append(_d)
    ...
    # Extract relationships using Qwen2-VL
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    try:
                        result = qwen.extract_object_relationships(
...
def main():
    ...
    print(f"Florence model: {args.florence_model}")
