#!/usr/bin/env python3
"""
Query VLM Scene - Example script for querying the scene graph using PaliGemma
...
def main():
    ...
    parser.add_argument(
        '--model',
        type=str,
        default='Qwen/PaliGemma-2B-Instruct',
        help='PaliGemma model to use'
    )
    parser.add_argument(
        '--device',
        type=str,
    ...
    print(f"\nInitializing PaliGemma model: {args.model}")
    paligemma = PaliGemmaModel(model_name=args.model, device=args.device)
    ...
    if args.query:
        # Single query mode
        print(f"\nQuery: {args.query}")
        answer = query_scene_single(paligemma, scene_graph, args.query, args.image)
        print(f"\nAnswer:\n{answer}\n")
    else:
        # Interactive mode
        query_scene_interactive(paligemma, scene_graph, args.image)
