"""
Build a scene graph using Vision-Language Models (Florence-2 + PaliGemma)
This replaces the YOLO+CLIP+LLaVA+GPT-4 pipeline with modern VLMs.

Key changes from build_scenegraph_cfslam.py:
- Florence-2 for captioning (replaces LLaVA)
- PaliGemma for caption refinement (replaces GPT-4)
- PaliGemma for relationship extraction (replaces GPT-4)
- Florence-2 embeddings for features (replaces CLIP)
"""
...
def extract_node_captions(args):
    """
    Extract captions using PaliGemma (replaces LLaVA and Florence-2).
    """
    from conceptgraph.vlm.paligemma_model import PaliGemmaModel
    ...
    print("PaliGemma initialized...")

    # Directories to save features and captions
    savedir_feat = Path(args.cachedir) / "cfslam_feat_qwen"
    savedir_feat.mkdir(exist_ok=True, parents=True)
    savedir_captions = Path(args.cachedir) / "cfslam_captions_qwen"
    savedir_captions.mkdir(exist_ok=True, parents=True)
    savedir_debug = Path(args.cachedir) / "cfslam_captions_qwen_debug"
    ...
    for idx_obj, obj in tqdm(enumerate(scene_map), total=len(scene_map)):
        ...
        for idx_det in tqdm(idx_most_conf, leave=False):
            ...
            # Generate caption using PaliGemma
            try:
                caption = paligemma.caption_image(image_crop_modified)
                console.print(f"[bold green]PaliGemma:[/bold green] {caption}")
                captions.append(caption)
                
                # Note: PaliGemma embeddings can be added here if needed
                # For now, we'll skip feature extraction since Qwen handles everything
        ...
        # Features not needed with PaliGemma (it handles everything internally)
    ...
    # Save captions
    with open(Path(args.cachedir) / "cfslam_paligemma_captions.json", "w", encoding="utf-8") as f:
...
def refine_node_captions(args):
    """
    Refine captions using PaliGemma (replaces GPT-4).
    """
    from conceptgraph.slam.slam_classes import MapObjectList
    from conceptgraph.vlm.paligemma_model import PaliGemmaModel

    # Load the captions
    caption_file = Path(args.cachedir) / "cfslam_paligemma_captions.json"
    ...
    responses_savedir = Path(args.cachedir) / "cfslam_paligemma_responses"
    ...
    for _i in trange(len(captions)):
        ...
        # Refine captions using PaliGemma
        try:
            result = paligemma.refine_caption(_caption["captions"])
    ...
    # Save all responses
    with open(Path(args.cachedir) / "cfslam_paligemma_responses.pkl", "wb") as f:
...
def build_scenegraph(args):
    ...
    response_dir = Path(args.cachedir) / "cfslam_paligemma_responses"
    responses = []
    object_tags = []
    ...
    for idx in range(len(scene_map)):
        ...
        with open(response_dir / f"{idx}.json", "r") as f:
            ...
            try:
                ...
                _d["response"] = {
                    'summary': f'PaliGemma response failed',
                    'possible_tags': ['invalid'],
                    'object_tag': 'invalid'
                }
            responses.append(_d)
    ...
    # Extract relationships using PaliGemma
    minimum_spanning_trees = []
    relations = []
    
    if len(labels) != 0:
        from conceptgraph.vlm.paligemma_model import PaliGemmaModel
        paligemma = PaliGemmaModel(model_name=args.paligemma_model, device=args.device)
        ...
        if not (Path(args.cachedir) / "cfslam_object_relations.json").exists():
            for componentidx, component in enumerate(components):
                ...
                for u, v in zip(
                    ...
                    try:
                        result = paligemma.extract_object_relationships(
...
def main():
    ...
    print(f"PaliGemma model: {args.paligemma_model}")
