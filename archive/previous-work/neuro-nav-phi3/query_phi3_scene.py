#!/usr/bin/env python3
"""
Query VLM Scene - Example script for querying the scene graph using Phi-3-Vision
...
def query_scene_interactive(qwen_model, scene_graph, image_path=None):
    ...
    while True:
        try:
            ...
            print("\nThinking...")
            answer = qwen_model.query_scene(query, context, image)
            print(f"\n{answer}\n")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Phi-3-Vision model to use'
    )
    parser.add_argument(
        '--device',
        type=str,
    ...
    print(f"\nInitializing Phi-3-Vision model: {args.model}")
    phi3 = Phi3VisionModel(model_name=args.model, device=args.device, load_in_8bit=True)
    ...
    if args.query:
        # Single query mode
        print(f"\nQuery: {args.query}")
        answer = query_scene_single(phi3, scene_graph, args.query, args.image)
        print(f"\nAnswer:\n{answer}\n")
    else:
        # Interactive mode
        query_scene_interactive(phi3, scene_graph, args.image)
